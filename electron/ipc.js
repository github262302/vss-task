...
import { startProcess, startProcessOutSide } from "vss/process/index";
...
const utils = {
  ...
  stopProcess(pid) {
    try {
      treeKill(pid, "SIGKILL", err => {
        ...
        if (err) {
          this.sendToMessage({
            content: "treeKill:" + JSON.stringify(err),
            title: "进程消息",
            type: "error",
          });
        }
  ...
  closeAddProject() {
    return closeAddProject();
  },
  reloadMainWindow() {
    return reloadMainWindow();
  },
  startProcessOutSide(/** @type { startProcessOutSideParams} */ data) {
    let common, args;
    if (data.type == "shell") {
      let com = data.command.split(" ");
      let arg = data.command.split(" ");
